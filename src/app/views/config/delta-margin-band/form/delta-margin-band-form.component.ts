...
@Component({
	selector: 'app-delta-margin-band-form',
	templateUrl: './delta-margin-band-form.component.html',
	styleUrl: './delta-margin-band-form.component.scss',
	standalone: false
})
export class DeltaMarginBandFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/config/delta-margin-band/list`;
	formAddUrl = `config/delta-margin-band/create`;
	formEditUrl = `config/delta-margin-band`;
	formErrorData: any = {};
	...
	ngOnInit () {
		...
		if (this.route.snapshot.paramMap.get("id") != null) {
			...
			this.router.navigate(["/config/delta-margin-band/list"], {queryParams: this.backUrlParams});
			this.toastr.warning('Details Not Found', 'Warning');
		}
	}
	dynamicFormField () {
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/delta-margin-band/list"]);
					} else {
						alert('Error');
					}
				},
