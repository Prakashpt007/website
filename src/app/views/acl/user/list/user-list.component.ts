...
export class UserListComponent {
	...
	async getData () {
		...
		this.router.navigate(["/acl/user/list"], {queryParams: params});
		this.listTableData = [];
		...
		this.genericHttp.getDataList(this.listUrl, params).subscribe(
			(response) => {
				if (response.status == 200 || response.status == true) {
					...
					this.componentListData.data.actions = this.componentListData.data.actions.filter((obj: {name: string;}) => obj.name == "edit");
