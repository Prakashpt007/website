...
@Component({
	selector: 'app-warehouse-farmer-form',
	templateUrl: './warehouse-farmer-form.component.html',
	styleUrl: './warehouse-farmer-form.component.scss',
	standalone: false,
	animations: [
		trigger("fade", [
			transition("void => *", [
...
export class WarehouseFarmerFormComponent {
		...
		this.genericHttp.storeObjectDataByPatchMethod({"slotId": slotId}, url).subscribe({
			next: (response: any) => {
				if (response.success == 200 || response.success == true) {
					...
					this.router.navigate(["/scheduling-qa/warehouse-farmer/list"]);
				} else {
