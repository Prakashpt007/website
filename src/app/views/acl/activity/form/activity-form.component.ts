...
export class ActivityFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/acl/activity/list`;
	formAddUrl = `roles/create`;
	formEditUrl = `roles`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/acl/activity/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/acl/activity/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
