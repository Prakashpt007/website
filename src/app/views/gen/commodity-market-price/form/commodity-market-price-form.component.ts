...
@Component({
	selector: 'app-commodity-market-price-form',
	templateUrl: './commodity-market-price-form.component.html',
	styleUrl: './commodity-market-price-form.component.scss',
	standalone: false
})
export class CommodityMarketPriceFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/commodity-market-price/list`;
	formAddUrl = `gen/commodity-market-price/create`;
	formEditUrl = `gen/commodity-market-price`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/commodity-market-price/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/commodity-market-price/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
