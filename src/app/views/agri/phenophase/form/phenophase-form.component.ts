...
@Component({
	selector: 'app-phenophase-form',
	templateUrl: './phenophase-form.component.html',
	styleUrls: ['./phenophase-form.component.scss'],
	standalone: false
})
export class PhenophaseFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/agri/phenophase/list`;
	formAddUrl = `agri/phenophase/create`;
	formEditUrl = `agri/phenophase`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/phenophase/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/phenophase/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
