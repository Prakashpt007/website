...
export class TenderListComponent {
	...
	editUrl: string = "/admin/bnt-error/tender/form";
	listUrl: string = "bnt/tender-bagging-tagging-error";
	...
	async getData () {
		...
		this.router.navigate(["/admin/bnt-error/tender/list"], {queryParams: params});
		this.listTableData = [];
	...
	componentActionRequest (event: any) {
		...
		let actionEventUrl = `${ event.actionUrl }/${ event.id }`;
		const data = {
			actionUrl: actionEventUrl,
			id: []
		};
		this.genericHttp.actionRequest(data).subscribe({
			next: (response: any) => {
