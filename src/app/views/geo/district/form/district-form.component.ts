...
export class DistrictFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/geo/district/list`;
	formAddUrl = `geo/district/create`;
	formEditUrl = `geo/district`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/geo/district/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/geo/district/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
