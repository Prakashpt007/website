...
@Component({
	selector: 'app-workflow-section-screen-form',
	templateUrl: './workflow-section-screen-form.component.html',
	styleUrl: './workflow-section-screen-form.component.scss',
	standalone: false
})
export class WorkflowSectionScreenFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/workflow-section-screen/list`;
	formAddUrl = `gen/workflow-section-screen/create`;
	formEditUrl = `gen/workflow-section-screen`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/workflow-section-screen/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/workflow-section-screen/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
