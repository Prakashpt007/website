...
@Component({
	selector: 'app-lot-default-quantity-form',
	templateUrl: './lot-default-quantity-form.component.html',
	styleUrl: './lot-default-quantity-form.component.scss',
	standalone: false
})
export class LotDefaultQuantityFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/config/lot-default-quantity/list`;
	formAddUrl = `config/lot-default-quantity/create`;
	formEditUrl = `config/lot-default-quantity`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/lot-default-quantity/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/lot-default-quantity/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
