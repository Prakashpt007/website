...
@Component({
	selector: 'app-pre-qual-duration-form',
	templateUrl: './pre-qual-duration-form.component.html',
	styleUrl: './pre-qual-duration-form.component.scss',
	standalone: false
})
export class PreQualDurationFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/config/pre-qual-duration/list`;
	formAddUrl = `config/pq-duration/create`;
	formEditUrl = `config/pq-duration`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/pre-qual-duration/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/pre-qual-duration/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
