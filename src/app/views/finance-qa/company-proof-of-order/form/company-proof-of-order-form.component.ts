...
@Component({
	selector: 'app-company-proof-of-order-form',
	templateUrl: './company-proof-of-order-form.component.html',
	styleUrl: './company-proof-of-order-form.component.scss',
	standalone: false,
	animations: [
		trigger("fade", [
			transition("void => *", [
...
export class CompanyProofOfOrderFormComponent {
	...
	formSubmit () {
		...
		if (this.form.invalid) {
			...
			if (this.pageId != 0 || this.pageId != undefined) {
				...
				formObjectData['orderValuePaisa'] = this.form.get('orderValuePaisa')?.value;
				formObjectData['projectName'] = this.form.get('projectName')?.value;

				this.genericHttp.storeObjectDataByPatchMethod(formObjectData, `${ this.pageId }/company-proof-of-order-completion-doc-validation/store-details`).subscribe({
					next: (response: any) => {
						if (response.status == 200 || response.success == true) {
							this.toastr.success(`Data Updated Successfully`, `Success`);
							// this.router.navigate(["/finance-qa/company-proof-of-order/list"]);
	...
	getData (pageId: number) {
		...
		this.genericHttp.getFormData(`${ pageId }/company-proof-of-order-completion-doc-validation/get-details`).subscribe({
			next: (response: any) => {
