...
@Component({
	selector: 'app-company-warehouse-form',
	templateUrl: './company-warehouse-form.component.html',
	styleUrl: './company-warehouse-form.component.scss',
	standalone: false,
	animations: [
		trigger("fade", [
			transition("void => *", [
...
export class CompanyWarehouseFormComponent {
			...
			if (this.pageId != 0 || this.pageId != undefined) {
				...
				this.genericHttp.storeObjectDataByPatchMethod(formObjectData, `${ this.pageId }/warehouse-doc-validation/store-details`).subscribe({
					next: (response: any) => {
						if (response.status == 200 || response.success == true) {
							this.toastr.success(`Data Updated Successfully`, `Success`);
							// this.router.navigate(["/warehouse-qa/company-warehouse/list"]);
