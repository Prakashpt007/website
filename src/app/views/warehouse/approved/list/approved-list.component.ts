...
export class ApprovedListComponent {
	...
	editUrl: string = "/warehouse/approved/form";
	listUrl: string = "warehouse/get-approved-list";
	...
	viewonlyFlagStatus: boolean = false;
	viewonlyRequestId!: number;
	...
	async getData () {
		...
		this.router.navigate(["/warehouse/approved/list"], {queryParams: params});
		this.listTableData = [];
		...
		this.genericHttp.getDataList(this.listUrl, params).subscribe(
			(response) => {
				if (response.status == 200 || response.status == true) {
					...
					// this.componentListData["data"]["actions"] = this.componentListData["data"]["actions"].filter((item: {name: string;}) => item.name != 'approve').filter((item: {name: string;}) => item.name != 'reject');
					this.componentListData["data"]["actions"] = [];
					this.componentListData["data"]["actions"].push({
						actionUrl: "get-approved-list/1/edit",
						name: "view-modal"
					});
					// this.componentListData["data"]["filters"][2].type = 'multiselect';
					this.filterComponentData = this.componentListData["data"]["filters"].filter((item: {name: string;}) => item.name != 'status');
	...
	getViewonlyFlagStatus (event: any) {
		this.viewonlyFlagStatus = event.viewonlyStatus;
		this.viewonlyRequestId = event.id;
		if (this.viewonlyFlagStatus) {

			this.modalService.open(this.viewonlyModal, {scrollable: true, centered: true, backdrop: 'static', size: 'lg'}).result.then(
				(result) => {
	...
	getModalResponse (res: any) {
		if (res.status == 500) {
			this.toastr.error(`${ res } Please contact administrator for more information`, `Error`);
		}
