...
export class WorkflowFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/workflow/list`;
	formAddUrl = `gen/workflow/create`;
	formEditUrl = `gen/workflow`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/workflow/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/workflow/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
