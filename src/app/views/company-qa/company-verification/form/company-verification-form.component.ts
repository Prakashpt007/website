...
@Component({
	selector: 'app-company-verification-form',
	templateUrl: './company-verification-form.component.html',
	styleUrl: './company-verification-form.component.scss',
	standalone: false,
	animations: [
		trigger("fade", [
			transition("void => *", [
...
export class CompanyVerificationFormComponent {
	...
	formSubmit () {
		...
		if (this.form.invalid) {
			...
			if (this.pageId != 0 || this.pageId != undefined) {
				...
				this.genericHttp.storeObjectDataByPatchMethod(formObjectData, `${ this.pageId }/company-verification/store-details`).subscribe({
					next: (response: any) => {
						if (response.status == 200 || response.success == true) {
							this.toastr.success(`Data Updated Successfully`, `Success`);
							// this.router.navigate(["/company-qa/verification/list"]);
	...
	getData (pageId: number) {
		...
		this.genericHttp.getFormData(`${ pageId }/company-verification/get-details`).subscribe({
			next: (response: any) => {
