...
@Component({
	selector: 'app-drk-form',
	templateUrl: './drk-form.component.html',
	styleUrl: './drk-form.component.scss',
	standalone: false
})
export class DrkFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/config/drk/list`;
	formAddUrl = `config/drk/create`;
	formEditUrl = `config/drk`;
	formErrorData: any = {};
	...
	ngOnInit () {
		...
		if (this.route.snapshot.paramMap.get("id") != null) {
			...
			this.router.navigate(["/config/drk/list"], {queryParams: this.backUrlParams});
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/drk/list"]);
					} else {
						alert('Error');
					}
				},
