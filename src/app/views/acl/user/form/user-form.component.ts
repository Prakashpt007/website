...
export class UserFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/acl/user/list`;
	formAddUrl = `user/create`;
	formEditUrl = `user`;
	formErrorData: any = {};
	...
	getFormValuesFromBuilder (event: any) {
		...
		if (this.formActivity == "add") {
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/acl/user/list"]);
					} else {
						alert('Error');
					}
				},
			...
			formData.append("userId", this.pageId);
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						this.toastr.success(`Data Updated Successfully`, `Success`);
						this.router.navigate(["/acl/user/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
