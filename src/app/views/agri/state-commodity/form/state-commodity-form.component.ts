...
@Component({
	selector: 'app-state-commodity-form',
	templateUrl: './state-commodity-form.component.html',
	styleUrls: ['./state-commodity-form.component.scss'],
	standalone: false
})
export class StateCommodityFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/agri/state-commodity/list`;
	formAddUrl = `agri/state-commodity/create`;
	formEditUrl = `agri/state-commodity`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/state-commodity/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/state-commodity/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
