...
@Component({
	selector: 'app-non-msp-price-form',
	templateUrl: './non-msp-price-form.component.html',
	styleUrl: './non-msp-price-form.component.scss',
	standalone: false
})
export class NonMspPriceFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/non-msp-price/list`;
	formAddUrl = `gen/non-msp-price/create`;
	formEditUrl = `gen/non-msp-price`;
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/non-msp-price/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/non-msp-price/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
