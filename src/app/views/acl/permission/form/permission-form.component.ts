...
export class PermissionFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/acl/permission/list`;
	formAddUrl = `permissions/create`;
	formEditUrl = `permissions`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/acl/permission/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/acl/permission/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
