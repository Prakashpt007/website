...
@Component({
	selector: 'app-template-variable-form',
	templateUrl: './template-variable-form.component.html',
	styleUrl: './template-variable-form.component.scss',
	standalone: false
})
export class TemplateVariableFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/alert/variable/list`;
	formAddUrl = `alert/variable/create`;
	formEditUrl = `alert/variable`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/alert/variable/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/alert/variable/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
