...
export class BankAccountFormComponent {
	...
	formSubmit () {
		...
		if (this.form.invalid) {
			...
			if (this.pageId != 0 || this.pageId != undefined) {
				...
				this.genericHttp.storeFormData(formData, `${ this.pageId }/bank-account-bnt-errors/store-details`).subscribe({
					next: (response: any) => {
	...
	getData (pageId: number) {
		...
		this.genericHttp.getFormData(`${ pageId }/bank-account-bnt-errors/get-details`).subscribe({
			next: (response: any) => {
