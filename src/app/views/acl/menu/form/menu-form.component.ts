...
export class MenuFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/acl/menu/list`;
	formAddUrl = `menu/menu/create`;
	formEditUrl = `menu/menu`;
	formErrorData: any = {};
	...
	ngOnInit () {
		...
		if (this.route.snapshot.paramMap.get("id") != null) {
			...
			if (this.pageId == 1) {
				...
				this.toastr.warning(`These Details Cannot Be Edited.`, `Warning`);
			} else {
	...
	dynamicFormEditField () {
		this.genericHttp.getFormData(this.formEditUrl + "/" + `${ this.pageId }` + "/edit").subscribe((response) => {
			if ((response.status == 200)) {
				if (this.pageId == 1) {
					response.data.formData.fields[1].validators['readonly'] = true;
					this.componentData = response.data.formData;
				} else {
					this.componentData = response.data.formData;
				}
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/acl/menu/list"], {queryParams: this.backUrlParams});
						sessionStorage.removeItem('menu');
					} else {
						alert('Error');
					}
