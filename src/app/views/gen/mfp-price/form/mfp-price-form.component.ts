...
@Component({
	selector: 'app-mfp-price-form',
	templateUrl: './mfp-price-form.component.html',
	styleUrl: './mfp-price-form.component.scss',
	standalone: false
})
export class MfpPriceFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/mfp-price/list`;
	formAddUrl = `gen/mfp-price/create`;
	formEditUrl = `gen/mfp-price`;
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/mfp-price/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/mfp-price/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
