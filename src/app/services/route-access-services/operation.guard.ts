...
export class OperationGuard {
	...
	canActivate (route: ActivatedRouteSnapshot,
		...
		if (!((typeof sessionStorage !== 'undefined') ? sessionStorage.getItem('userActivities')?.replaceAll(' ', '').split(',') : undefined)?.includes('ALL_OPERATIONAL_DATA')) {
			this.toastr.info(`You are not authorized to view this page. Required => ALL_OPERATIONAL_DATA`, `Access Denied`);
