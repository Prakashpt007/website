...
@Component({
	selector: 'app-commodity-pmp-decrease-percent-form',
	templateUrl: './commodity-pmp-decrease-percent-form.component.html',
	styleUrl: './commodity-pmp-decrease-percent-form.component.scss',
	standalone: false
})
export class CommodityPmpDecreasePercentFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/config/commodity-pmp-decrease-percent/list`;
	formAddUrl = `config/commodity-pmp-decrease-percent/create`;
	formEditUrl = `config/commodity-pmp-decrease-percent`;
	formErrorData: any = {};
		...
		this.genericHttp.getFormData(this.formEditUrl + "/" + `${ this.pageId }` + "/edit").subscribe({
			next: (response: any) => {
				if (response.status == 200) {
					...
					// response.data.formData.fields[3].type = 'customNumber';
					// response.data.formData.fields[3].validators['min'] = 1;
					// response.data.formData.fields[3].validators['max'] = 100;
					this.componentData = response.data.formData;
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/commodity-pmp-decrease-percent/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/commodity-pmp-decrease-percent/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
