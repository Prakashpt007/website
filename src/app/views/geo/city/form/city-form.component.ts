...
export class CityFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/geo/city/list`;
	formAddUrl = `geo/city/create`;
	formEditUrl = `geo/city`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/geo/city/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/geo/city/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
