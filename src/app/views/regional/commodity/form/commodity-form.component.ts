...
@Component({
	...
	styleUrl: './commodity-form.component.scss',
	standalone: false
})
export class CommodityFormComponent {
	...
	commodity_id!: any;
	backUrlParams!: any;
	backUrl = `/regional/commodity/list`;
	formAddUrl = `regional/commodity/create`;
	formEditUrl = `regional/commodity`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/regional/commodity/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/regional/commodity/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
