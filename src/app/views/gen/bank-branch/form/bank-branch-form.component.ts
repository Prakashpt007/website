...
export class BankBranchFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/bank-branch/list`;
	formAddUrl = `gen/bank-branch/create`;
	formEditUrl = `gen/bank-branch`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/bank-branch/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/bank-branch/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
