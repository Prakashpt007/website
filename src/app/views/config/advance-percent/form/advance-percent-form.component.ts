...
@Component({
	selector: 'app-advance-percent-form',
	templateUrl: './advance-percent-form.component.html',
	styleUrl: './advance-percent-form.component.scss',
	standalone: false
})
export class AdvancePercentFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/config/advance-percent/list`;
	formAddUrl = `config/advance-percent/create`;
	formEditUrl = `config/advance-percent`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/advance-percent/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/advance-percent/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
