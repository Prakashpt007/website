...
@Component({
	selector: 'app-fees-and-taxes-form',
	templateUrl: './fees-and-taxes-form.component.html',
	styleUrl: './fees-and-taxes-form.component.scss',
	standalone: false
})
export class FeesAndTaxesFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/fees-and-taxes/list`;
	formAddUrl = `gen/fees-and-taxes/create`;
	formEditUrl = `gen/fees-and-taxes`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/fees-and-taxes/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/fees-and-taxes/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
