...
export class UserKycFormComponent {
	...
	formSubmit () {
		...
		if (this.form.invalid) {
			...
			if (this.pageId != 0 || this.pageId != undefined) {
				...
				this.genericHttp.storeObjectDataByPatchMethod(formObjectData, `${ this.pageId }/basic-kyc-doc-validation/store-details`).subscribe({
					next: (response: any) => {
						if (response.status == 200 || response.success == true) {
							this.toastr.success(`Data Updated Successfully`, `Success`);
							// this.router.navigate(["/basic-kyc/user-kyc/list"]);
	...
	getData (pageId: number) {
		...
		this.genericHttp.getFormData(`${ pageId }/basic-kyc-doc-validation/get-details`).subscribe({
			next: (response: any) => {
