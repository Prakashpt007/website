...
@Component({
	selector: 'app-phenophase-duration-form',
	templateUrl: './phenophase-duration-form.component.html',
	styleUrls: ['./phenophase-duration-form.component.scss'],
	standalone: false
})
export class PhenophaseDurationFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/agri/phenophase-duration/list`;
	formAddUrl = `agri/phenophase-duration/create`;
	formEditUrl = `agri/phenophase-duration`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/phenophase-duration/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/phenophase-duration/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
