...
export class LanguageFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/language/list`;
	formAddUrl = `gen/language/create`;
	formEditUrl = `gen/language`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/language/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/language/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
