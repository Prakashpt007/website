...
@Component({
	selector: 'app-visit-time-form',
	templateUrl: './visit-time-form.component.html',
	styleUrls: ['./visit-time-form.component.scss'],
	standalone: false
})
export class VisitTimeFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/regional/visit-time/list`;
	formAddUrl = `regional/visit-time/create`;
	formEditUrl = `regional/visit-time`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/regional/visit-time/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/regional/visit-time/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
