...
@Component({
	selector: 'app-geo-plot-form',
	templateUrl: './geo-plot-form.component.html',
	styleUrl: './geo-plot-form.component.scss',
	standalone: false,
	animations: [
		trigger("fade", [
			transition("void => *", [
...
export class GeoPlotFormComponent {
	...
	documentedArea: any;
	plottedArea: any;
	resultClass: string = "btn-primary";
	...
	getData (caseID: string) {
		...
		this.genericHttp.getFormData(`${ caseID }/geo-plotting-validation/get-details`).subscribe({
			next: (response: any) => {
				if (response.success == 200 || response.success == true) {
					this.documentedArea = Number(response.data.documentedArea.value);
					this.plottedArea = Number(response.data.plottedArea.value);
					...
					this.geoPoints = JSON.parse(response.data.geoPoints);


					let result = this.documentedArea - this.plottedArea;
					if (result > 0) {
						this.resultClass = "btn-success";
					} else if (result == 0) {
						this.resultClass = "btn-success";
					} else {
						this.resultClass = "btn-danger";
					}
	...
	reject (requestId: any, reason: any) {
		this.modalService.open(this.rejectModal).result.then(
			(result) => {
				...
				this.genericHttp.rejectDoc(`${ requestId }/geo-plotting-validation/reject`, data).subscribe({
					next: (response: any) => {
						if (response.status == '200' || response.success) {
							this.toastr.success("Rejected Successfully", "Success");
							this.router.navigate(["/geo-qa/geo-plot/list"]);
						} else {
	...
	approve (requestId: any) {
		this.modalService.open(this.approveModal).result.then(
			(result) => {
				...
				this.genericHttp.approvedDoc(`${ requestId }/geo-plotting-validation/approve`).subscribe({
					next: (response: any) => {
						if (response.status == '200' || response.success) {
