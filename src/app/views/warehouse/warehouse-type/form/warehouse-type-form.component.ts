...
@Component({
	...
	styleUrls: ['./warehouse-type-form.component.scss'],
	standalone: false
})
export class WarehouseTypeFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/warehouse/type/list`;
	formAddUrl = `warehouse/warehouse-type/create`;
	formEditUrl = `warehouse/warehouse-type`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/warehouse/type/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/warehouse/type/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
