...
@Component({
	selector: 'app-quantity-threshold-form',
	templateUrl: './quantity-threshold-form.component.html',
	styleUrl: './quantity-threshold-form.component.scss',
	standalone: false
})
export class QuantityThresholdFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/quantity-threshold/list`;
	formAddUrl = `gen/quantity-threshold/create`;
	formEditUrl = `gen/quantity-threshold`;
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/quantity-threshold/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/quantity-threshold/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
