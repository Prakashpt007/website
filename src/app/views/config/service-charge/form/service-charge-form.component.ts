...
@Component({
	...
	styleUrl: './service-charge-form.component.scss',
	standalone: false
})
export class ServiceChargeFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/config/service-charge/list`;
	formAddUrl = `config/service-charge/create`;
	formEditUrl = `config/service-charge`;
	formErrorData: any = {};
	...
	ngOnInit () {
		...
		if (this.route.snapshot.paramMap.get("id") != null) {
			...
			this.router.navigate(["/config/service-charge/list"], {queryParams: this.backUrlParams});
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/service-charge/list"]);
					} else {
						alert('Error');
					}
				},
