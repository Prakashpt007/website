...
@Component({
	...
	styleUrl: './company-role-form.component.scss',
	standalone: false
})
export class CompanyRoleFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/company-role/list`;
	formAddUrl = `gen/company-role/create`;
	formEditUrl = `gen/company-role`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/company-role/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/company-role/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
