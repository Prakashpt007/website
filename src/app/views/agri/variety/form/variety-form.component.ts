...
export class VarietyFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/agri/variety/list`;
	formAddUrl = `agri/variety/create`;
	formEditUrl = `agri/variety`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/variety/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/variety/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
