...
@Component({
	selector: 'app-tender-type-form',
	templateUrl: './tender-type-form.component.html',
	styleUrls: ['./tender-type-form.component.scss'],
	standalone: false
})
export class TenderTypeFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/tender-type/list`;
	formAddUrl = `gen/tender-type/create`;
	formEditUrl = `gen/tender-type`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/tender-type/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/tender-type/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
