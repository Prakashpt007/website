...
@Component({
	selector: 'app-seller-default-management-list',
	standalone: false,
	templateUrl: './seller-default-management-list.component.html',
	styleUrl: './seller-default-management-list.component.scss'
})
export class SellerDefaultManagementListComponent {
	currentIndex = 1;
	page = 1;
	orderBy: string = '';
	order: string = '';
	...
	listUrl: string = "intervention/seller-default-management";
	...
	async getData () {
		...
		this.router.navigate(["/intervention/seller-default-management/list"], {queryParams: params});
		this.listTableData = [];
	...
	componentActionRequest (event: any) {
		...
		this.genericHttp.actionGetRequest(`${ event.actionUrl }/${ event.id }`).subscribe({
			next: (response: any) => {
