...
@Component({
	...
	templateUrl: './holiday-calendar-form.component.html',
	styleUrls: ['./holiday-calendar-form.component.scss'],
	standalone: false
})
export class HolidayCalendarFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/holiday-calendar/list`;
	formAddUrl = `gen/holiday-calendar/create`;
	formEditUrl = `gen/holiday-calendar`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/holiday-calendar/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/holiday-calendar/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
