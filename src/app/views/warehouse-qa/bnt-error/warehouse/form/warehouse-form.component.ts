...
export class WarehouseFormComponent {
	...
	formSubmit () {
		...
		if (this.form.invalid) {
			...
			if (this.pageId != 0 || this.pageId != undefined) {
				...
				this.genericHttp.storeObjectDataByPatchMethod(formObjectData, `${ this.pageId }/warehouse-bnt-errors/store-details`).subscribe({
					next: (response: any) => {
						if (response.status == 200 || response.success == true) {
							this.toastr.success(`Data Updated Successfully`, `Success`);
							// this.router.navigate(["/warehouse-qa/bnt-error/warehouse/list"]);
	...
	getData (pageId: number) {
		...
		this.genericHttp.getFormData(`${ pageId }/warehouse-bnt-errors/get-details`).subscribe({
			next: (response: any) => {
