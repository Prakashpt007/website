...
export class RegionFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/geo/region/list`;
	formAddUrl = `geo/region/create`;
	formEditUrl = `geo/region`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/geo/region/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/geo/region/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
