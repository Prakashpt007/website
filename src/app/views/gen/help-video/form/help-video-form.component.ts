...
@Component({
	selector: 'app-help-video-form',
	templateUrl: './help-video-form.component.html',
	styleUrls: ['./help-video-form.component.scss'],
	standalone: false
})
export class HelpVideoFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/help-video/list`;
	formAddUrl = `gen/help-video/create`;
	formEditUrl = `gen/help-video`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/help-video/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/help-video/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
