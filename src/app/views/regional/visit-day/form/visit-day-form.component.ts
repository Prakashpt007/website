...
@Component({
	selector: 'app-visit-day-form',
	templateUrl: './visit-day-form.component.html',
	styleUrls: ['./visit-day-form.component.scss'],
	standalone: false
})
export class VisitDayFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/regional/visit-day/list`;
	formAddUrl = `regional/visit-day/create`;
	formEditUrl = `regional/visit-day`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/regional/visit-day/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/regional/visit-day/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
