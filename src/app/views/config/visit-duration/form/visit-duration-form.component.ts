...
@Component({
	selector: 'app-visit-duration-form',
	templateUrl: './visit-duration-form.component.html',
	styleUrl: './visit-duration-form.component.scss',
	standalone: false
})
export class VisitDurationFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/config/visit-duration/list`;
	formAddUrl = `config/visit-duration/create`;
	formEditUrl = `config/visit-duration`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/visit-duration/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/visit-duration/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
