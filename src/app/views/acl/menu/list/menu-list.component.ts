...
export class MenuListComponent {
	...
	editUrl: string = "/acl/menu/form";
	listUrl: string = "menu/menu";
	...
	async getData () {
		...
		this.router.navigate(["/acl/menu/list"], {queryParams: params});
		this.listTableData = [];
		...
		this.genericHttp.getDataList(this.listUrl, params).subscribe(
			(response) => {
				if (response.status == 200 || response.status == true) {
					response.data.rows = response.data.rows.filter((row: {menuOrder: string | null;}) => row.menuOrder !== "0" && row.menuOrder !== null && row.menuOrder !== "");
					...
					// // Following code for disabled list item for click
					// this.componentListData["data"]["rows"] = this.componentListData["data"]["rows"].filter((item: any) => !(item.name === "Dashboard" && item.id === 1));
					this.componentListData["data"]["rows"] = this.componentListData["data"]["rows"].map((item: {name: string; id: number;}) => {
						if (item.name === "Dashboard" && item.id === 1) {
