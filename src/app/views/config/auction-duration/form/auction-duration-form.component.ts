...
@Component({
	selector: 'app-auction-duration-form',
	templateUrl: './auction-duration-form.component.html',
	styleUrl: './auction-duration-form.component.scss',
	standalone: false
})
export class AuctionDurationFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/config/auction-duration/list`;
	formAddUrl = `config/auction-duration/create`;
	formEditUrl = `config/auction-duration`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/auction-duration/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/auction-duration/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
