...
@Component({
	...
	styleUrls: ['./crop-type-form.component.scss'],
	standalone: false
})
export class CropTypeFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/agri/crop-type/list`;
	formAddUrl = `agri/crop-type/create`;
	formEditUrl = `agri/crop-type`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/crop-type/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/crop-type/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
