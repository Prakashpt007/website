...
@Component({
	selector: 'app-agm-form',
	templateUrl: './agm-form.component.html',
	styleUrl: './agm-form.component.scss',
	standalone: false
})
export class AgmFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/config/agm/list`;
	formAddUrl = `config/agm/create`;
	formEditUrl = `config/agm`;
	formErrorData: any = {};
	...
	ngOnInit () {
		...
		if (this.route.snapshot.paramMap.get("id") != null) {
			...
			this.router.navigate(["/config/agm/list"], {queryParams: this.backUrlParams});
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/config/agm/list"]);
					} else {
						alert('Error');
					}
				},
