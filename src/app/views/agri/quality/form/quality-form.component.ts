...
export class QualityFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/agri/quality/list`;
	formAddUrl = `agri/quality/create`;
	formEditUrl = `agri/quality`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/quality/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/quality/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
