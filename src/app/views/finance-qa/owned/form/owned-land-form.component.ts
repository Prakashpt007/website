...
@Component({
	selector: 'app-owned-land-form',
	templateUrl: './owned-land-form.component.html',
	styleUrl: './owned-land-form.component.scss',
	standalone: false,
	animations: [
		trigger("fade", [
			transition("void => *", [
...
export class OwnedLandFormComponent {
	...
	formSubmit () {
		...
		if (this.form.invalid) {
			...
			if (this.pageId != 0 || this.pageId != undefined) {
				...
				this.genericHttp.storeObjectDataByPatchMethod(formObjectData, `${ this.pageId }/land-doc-validation/store-details`).subscribe({
					next: (response: any) => {
						if (response.status == 200 || response.success == true) {
							this.toastr.success(`Data Updated Successfully`, `Success`);
							// this.router.navigate(["/finance-qa/owned-land/list"]);
	...
	getData (pageId: number) {
		...
		this.genericHttp.getFormData(`${ pageId }/land-doc-validation/get-details`).subscribe({
			next: (response: any) => {
