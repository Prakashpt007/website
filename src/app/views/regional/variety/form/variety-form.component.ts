...
export class VarietyFormComponent {
	...
	region_id!: any;
	commodity_id!: any;
	parent_variety_id!: any;
	backUrlParams!: any;
	backUrl = `/regional/variety/list`;
	formAddUrl = `regional/variety/create`;
	formEditUrl = `regional/variety`;
	formErrorData: any = {};
	...
	ngOnInit () {
		...
		if (this.route.snapshot.paramMap.get("id_1") != null && this.route.snapshot.paramMap.get("id_2") != null && this.route.snapshot.paramMap.get("id_3") != null) {
			...
			this.parent_variety_id = Number(`${ this.route.snapshot.paramMap.get("id_3") }`);
	...
	dynamicFormEditField () {
		this.genericHttp.getFormData(this.formEditUrl + "/" + `${ this.region_id }/${ this.commodity_id }/${ this.parent_variety_id }` + "/edit").subscribe({
			next: (response: any) => {
				if (response.status == 200) {
					this.componentData = response.data.formData;
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/regional/variety/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/regional/variety/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
