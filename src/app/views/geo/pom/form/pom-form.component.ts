...
@Component({
	selector: 'app-pom-form',
	templateUrl: './pom-form.component.html',
	styleUrl: './pom-form.component.scss',
	standalone: false
})
export class PomFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/geo/pom/list`;
	formAddUrl = `geo/pom/create`;
	formEditUrl = `geo/pom`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/geo/pom/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/geo/pom/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
