...
export class ChannelFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/alert/channel/list`;
	formAddUrl = `alert/channel/create`;
	formEditUrl = `alert/channel`;
	formErrorData: any = {};
	...
	dynamicFormField () {
		this.genericHttp.getFormData(this.formAddUrl).subscribe({
			// this.genericHttp.getDummyData('assets/jsons/channel-add.json').subscribe({
			next: (response: any) => {
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/alert/channel/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/alert/channel/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
