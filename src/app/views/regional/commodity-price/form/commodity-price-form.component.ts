...
@Component({
	...
	templateUrl: './commodity-price-form.component.html',
	styleUrl: './commodity-price-form.component.scss',
	standalone: false
})
export class CommodityPriceFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/regional/commodity-price/list`;
	formAddUrl = `regional/commodity-price/create`;
	formEditUrl = `regional/commodity-price`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/regional/commodity-price/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/regional/commodity-price/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
