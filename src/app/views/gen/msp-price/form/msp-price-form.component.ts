...
@Component({
	selector: 'app-msp-price-form',
	templateUrl: './msp-price-form.component.html',
	styleUrl: './msp-price-form.component.scss',
	standalone: false
})
export class MspPriceFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/msp-price/list`;
	formAddUrl = `gen/msp-price/create`;
	formEditUrl = `gen/msp-price`;
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/msp-price/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/msp-price/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
