...
@Component({
	selector: 'app-commodity-benchmark-market-form',
	templateUrl: './commodity-benchmark-market-form.component.html',
	styleUrl: './commodity-benchmark-market-form.component.scss',
	standalone: false
})
export class CommodityBenchmarkMarketFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/commodity-benchmark-market/list`;
	formAddUrl = `gen/commodity-benchmark-market/create`;
	formEditUrl = `gen/commodity-benchmark-market`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/commodity-benchmark-market/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/commodity-benchmark-market/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
