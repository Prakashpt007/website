...
@Component({
	selector: 'app-uom-type-form',
	templateUrl: './uom-type-form.component.html',
	styleUrls: ['./uom-type-form.component.scss'],
	standalone: false
})
export class UomTypeFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/uom-type/list`;
	formAddUrl = `gen/uom-type/create`;
	formEditUrl = `gen/uom-type`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/uom-type/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/uom-type/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
