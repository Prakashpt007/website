...
@Component({
	selector: 'app-commodity-spec-form',
	templateUrl: './commodity-spec-form.component.html',
	styleUrl: './commodity-spec-form.component.scss',
	standalone: false
})
export class CommoditySpecFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/regional/commodity-spec/list`;
	formAddUrl = `regional/commodity-spec/create`;
	formEditUrl = `regional/commodity-spec`;
	formErrorData: any = {};
	...
	getFormValuesFromBuilder (event: any) {
		...
		if (this.formActivity == "add") {
			...
			// 			this.router.navigate(["/regional/commodity-spec/list"]);
			// 		} else {
			// 			alert('Error');
			// 		}
			// 	},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/regional/commodity-spec/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
