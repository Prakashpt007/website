...
export class RejectionReasonFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/rejection-reason/list`;
	formAddUrl = `gen/rejection-reason/create`;
	formEditUrl = `gen/rejection-reason`;
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/rejection-reason/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/rejection-reason/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
	...
	getBulkInsertValues (event: any) {
		...
		// this.genericHttp.bulkFormData(event.storeUrl, data).subscribe((response) => {
		//   if (response.status == 200 && response.success) {
		//     this.toastr.success(`Data Created Successfully`, `Success`);
		...
		//   } else {
		//     this.toastr.error(`${response.errors[0].errorMessage}`, `${response.message}`)
		//   }
		// });
	}
}
