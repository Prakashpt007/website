...
@Component({
	selector: 'app-represent-commodity-form',
	templateUrl: './represent-commodity-form.component.html',
	styleUrl: './represent-commodity-form.component.scss',
	standalone: false
})
export class RepresentCommodityFormComponent {
	...
	commodity_id!: any;
	backUrlParams!: any;
	backUrl = `/regional/represent-commodity/list`;
	formAddUrl = `regional/represent-commodity/create`;
	formEditUrl = `regional/represent-commodity`;
	formErrorData: any = {};
	...
	ngOnInit () {
		...
		if (this.route.snapshot.paramMap.get("id_1") != null && this.route.snapshot.paramMap.get("id_2") != null) {
			this.region_id = Number(`${ this.route.snapshot.paramMap.get("id_1") }`);
			this.commodity_id = Number(`${ this.route.snapshot.paramMap.get("id_2") }`);
	...
	dynamicFormEditField () {
		this.genericHttp.getFormData(this.formEditUrl + "/" + `${ this.region_id }/${ this.commodity_id }` + "/edit").subscribe({
			next: (response: any) => {
				if (response.status == 200) {
					this.componentData = response.data.formData;
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/regional/represent-commodity/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/regional/represent-commodity/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
