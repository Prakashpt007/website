...
@Component({
	selector: 'app-commodity-quality-parameter-range-add',
	templateUrl: './commodity-quality-parameter-range-add.component.html',
	styleUrl: './commodity-quality-parameter-range-add.component.scss',
	standalone: false
})
export class CommodityQualityParameterRangeAddComponent {
	...
	commodityChangeEvent (event: any) {
		this.form.get('varietyId')?.setValue('');
		...
		this.getVariety(event.target.value).subscribe({
			next: (response: any[]) => {
	...
	varietyChangeEvent (event: any) {
		this.resetParams();
		this.getParams({commodityId: this.form.get('commodityId')?.value, varietyId: this.form.get('varietyId')?.value});
	}
	...
	getParams (data: {commodityId: any, varietyId: any;}) {

		if (data.commodityId != '' && data.varietyId != '') {
			let commodityId = Number(data.commodityId);
			let varietyId = Number(data.varietyId);
			this.changeDetectorRef.detectChanges();
			...
			this.getFormDataByCommodityAndVarietyId(commodityId, varietyId).subscribe({
				next: (response) => {


					this.params[0].value = [...response.parameterValues];

					// let optionIds: number[] = [];
					response.parameters.forEach((item: any) => {
						// console.log();
						...
						// console.log(response.parameterValues.includes(item.options.value));

						// if (optionIds.includes(response.parameterValues)) {
						// 	console.log('yes');
						...
						// 	this.params[0].value.concat.apply([], val);
						// });
					...
					this.parameterData = response.parameterData;
	...
	getFormDataByCommodityAndVarietyId (commodityId: number, varietyId: number): Observable<any> {
		const dataSubject: Subject<any> = new Subject<any>();
		const data$: Observable<any> = dataSubject.asObservable();

		// Load Params
		this.genericHttp.getFormData(`agri/parameter-ranges/${ commodityId }/${ varietyId }`).subscribe({
			next: (response) => {

				if (response.status == 200 || response.success) {
					this.pageData = response.data;

					dataSubject.next(this.pageData); // Emit the params value
					dataSubject.complete(); // Complete the Observable
			...
			error: (err: any) => {
				this.toastr.error(err.error.message, `Error`);
				dataSubject.error(err); // Pass the error to the Observable
			},
			complete: () => {
				// console.log('completed');
			}
			...
			this.genericHttp.storeFormData(formData, `agri/parameter-ranges/save`).subscribe({
				next: (response: any) => {
					if (response.status == 200 || response.success == true) {
						...
						this.router.navigate(["agri/quality-parameter-range/list"]);
					} else {
						alert('Error');
					}
				},
				...
				if (this.pageData.parameterValues.includes(parameterId)) {
					...
					this.genericHttp.deleteAction(`agri/quality-parameter-range/${ parameterId }`).subscribe({
						next: (response: any) => {
							...
							this.getParams({commodityId: this.pageData.commodityId, varietyId: this.pageData.varietyId});
						},
						error: (err: any) => {
							this.toastr.error(`Please contact administrator for more information`, `Error`);
						},
