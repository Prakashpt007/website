...
export class TemplateFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/alert/template/list`;
	formAddUrl = `alert/template/create`;
	formEditUrl = `alert/template`;
	formErrorData: any = {};
	...
	ngOnInit () {
		...
		if (this.route.snapshot.paramMap.get("id") != null) {
			...
			this.router.navigate(["/alert/template/list"], {queryParams: this.backUrlParams});
	...
	dynamicFormEditField () {
		this.genericHttp.getFormData(this.formEditUrl + "/" + `${ this.pageId }` + "/edit").subscribe({
			// this.genericHttp.getDummyData('assets/jsons/template-edit.json').subscribe({
			next: (response: any) => {
