...
@Component({
	...
	styleUrls: ['./approved-form.component.scss'],
	standalone: false
})
export class ApprovedFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/warehouse/approved/list`;
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/warehouse/approved/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/warehouse/approved/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
