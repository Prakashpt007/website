...
export class CommodityFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/agri/commodity/list`;
	formAddUrl = `agri/commodity/create`;
	formEditUrl = `agri/commodity`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/commodity/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/commodity/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
