...
export class ContinentFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/geo/continent/list`;
	formAddUrl = `geo/continent/create`;
	formEditUrl = `geo/continent`;
	formErrorData: any = {};
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/geo/continent/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
