...
@Component({
	selector: 'app-workflow-section-form',
	templateUrl: './workflow-section-form.component.html',
	styleUrl: './workflow-section-form.component.scss',
	standalone: false
})
export class WorkflowSectionFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/workflow-section/list`;
	formAddUrl = `gen/workflow-section/create`;
	formEditUrl = `gen/workflow-section`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/workflow-section/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/workflow-section/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
