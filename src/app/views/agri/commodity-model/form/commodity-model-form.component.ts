...
@Component({
	selector: 'app-commodity-model-form',
	templateUrl: './commodity-model-form.component.html',
	styleUrls: ['./commodity-model-form.component.scss'],
	standalone: false
})
export class CommodityModelFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/agri/commodity-model/list`;
	formAddUrl = `agri/commodity-model/create`;
	formEditUrl = `agri/commodity-model`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/commodity-model/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/commodity-model/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
