...
@Component({
	...
	styleUrls: ['./sub-menu-form.component.scss'],
	standalone: false
})
export class SubMenuFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/acl/submenu/list`;
	formAddUrl = `menu/sub-menu/create`;
	formEditUrl = `menu/sub-menu`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/acl/submenu/list"]);
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/acl/submenu/list"], {queryParams: this.backUrlParams});
						sessionStorage.removeItem('menu');
					} else {
						alert('Error');
					}
