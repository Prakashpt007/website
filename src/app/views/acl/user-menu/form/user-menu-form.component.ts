...
@Component({
	selector: 'app-user-menu-form',
	templateUrl: './user-menu-form.component.html',
	styleUrls: ['./user-menu-form.component.scss'],
	standalone: false
})
export class UserMenuFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/acl/user-menu/list`;
	formAddUrl = `menu/activity-menu/create`;
	formEditUrl = `menu/activity-menu`;
	formErrorData: any = {};
	...
	dynamicFormField () {
		this.genericHttp.getFormData(this.formAddUrl).subscribe((response) => {
			if (response.status == 200) {

				// response.data.formData.fields.options = response.data.formData.fields.options = [];
				if (response.status == 200) {
	...
	dynamicFormEditField () {
		this.genericHttp.getFormData(this.formEditUrl + "/" + `${ this.pageId }` + "/edit").subscribe({
			next: (response: any) => {
				if (response.status == 200) {
					response.data.formData.fields.filter((item: {name: string; validators: {[x: string]: boolean;};}) => {
						if (item.name == 'activity_id') {
							item.validators['readonly'] = true;
						}
					});
					this.componentData = response.data.formData;
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/acl/user-menu/list"]);
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/acl/user-menu/list"], {queryParams: this.backUrlParams});
						sessionStorage.removeItem('menu');
					} else {
						alert('Error');
					}
