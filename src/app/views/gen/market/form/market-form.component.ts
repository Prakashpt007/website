...
export class MarketFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/market/list`;
	formAddUrl = `gen/market/create`;
	formEditUrl = `gen/market`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/market/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/market/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
