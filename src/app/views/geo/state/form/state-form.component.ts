...
export class StateFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/geo/state/list`;
	formAddUrl = `geo/state/create`;
	formEditUrl = `geo/state`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/geo/state/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/geo/state/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
