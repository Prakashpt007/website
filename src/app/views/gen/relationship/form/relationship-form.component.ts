...
export class RelationshipFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/relationship/list`;
	formAddUrl = `gen/relationship/create`;
	formEditUrl = `gen/relationship`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/relationship/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/relationship/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
