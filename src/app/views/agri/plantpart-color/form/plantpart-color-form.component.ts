...
@Component({
	selector: 'app-plantpart-color-form',
	templateUrl: './plantpart-color-form.component.html',
	styleUrls: ['./plantpart-color-form.component.scss'],
	standalone: false
})
export class PlantpartColorFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/agri/plantpart-color/list`;
	formAddUrl = `agri/plantpart-color/create`;
	formEditUrl = `agri/plantpart-color`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/plantpart-color/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/plantpart-color/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
