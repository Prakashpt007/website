...
@Component({
	selector: 'app-commodity-color-form',
	templateUrl: './commodity-color-form.component.html',
	styleUrl: './commodity-color-form.component.scss',
	standalone: false
})
export class CommodityColorFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/agri/commodity-color/list`;
	formAddUrl = `agri/commodity-color/create`;
	formEditUrl = `agri/commodity-color`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/commodity-color/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/commodity-color/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
