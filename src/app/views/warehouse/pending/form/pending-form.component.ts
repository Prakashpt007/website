...
export class PendingFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/warehouse/pending/list`;
	formAddUrl = `warehouse/get-approved-list/create`;
	formEditUrl = `warehouse/get-approved-list`;
	formErrorData: any = {};
	...
	dynamicFormEditField () {
		this.genericHttp
			.getFormData(this.formEditUrl + "/" + `${ this.pageId }` + "/edit")
			.subscribe((response) => {
				if ((response.status == 200)) {
					this.componentData = response.data.formData;
					this.bulkDataComponent = [];
					this.existingWhList = response.data.existingWhList;
					// this.existingWhList = [
					// 	{
					// 		"name": "existingWh",
					// 		"label": "Existing Warehouses",
					// 		"type": "radio",
					// 		"storeUrl": "warehouse/swaping",
					// 		"validators": {
					// 			"required": true
					// 		},
					// 		"options": [
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/warehouse/pending/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/warehouse/pending/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
	...
	getOtherFieldsSubmit (event: any) {
		console.log(event);
		const data = {
			"currentWarehouseId": this.pageId,
			"existingWarehouseId": event.data.existingWh
		};

		this.genericHttp.storeObjectDataByPatchMethod(data, event.storeUrl).subscribe({
			next: (response: any) => {
				if (response.status == 200 && response.success) {
					this.toastr.success(`Data Updated Successfully`, `Success`);
					this.router.navigate(["/warehouse/pending/list"]);
