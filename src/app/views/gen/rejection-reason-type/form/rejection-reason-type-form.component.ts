...
@Component({
	selector: 'app-rejection-reason-type-form',
	templateUrl: './rejection-reason-type-form.component.html',
	styleUrls: ['./rejection-reason-type-form.component.scss'],
	standalone: false
})
export class RejectionReasonTypeFormComponent {
	showFormBuilder: boolean = false;
	componentData: any;
	bulkDataComponent: any;
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/gen/rejection-reason-type/list`;
	formAddUrl = `gen/rejection-reason-type/create`;
	formEditUrl = `gen/rejection-reason-type`;
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/rejection-reason-type/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/gen/rejection-reason-type/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
