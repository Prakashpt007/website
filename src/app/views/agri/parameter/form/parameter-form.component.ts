...
export class ParameterFormComponent {
	...
	pageId!: any;
	backUrlParams!: any;
	backUrl = `/agri/parameter/list`;
	formAddUrl = `agri/parameter/create`;
	formEditUrl = `agri/parameter`;
	formErrorData: any = {};
			...
			this.genericHttp.storeFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/parameter/list"]);
					} else {
						alert('Error');
					}
				},
			...
			this.genericHttp.editFormData(formData, event['storeUrl']).subscribe({
				next: (response: any) => {
					if (response.status == 200 && response.success) {
						...
						this.router.navigate(["/agri/parameter/list"], {queryParams: this.backUrlParams});
					} else {
						alert('Error');
					}
				},
